...
import eventsByBand from '@/queries/eventsByBand'
...
export default {
  ...
  async asyncData ({ $axios, $payloadURL, route, app, params, payload }) {
    ...
    response.eventsByBand = await client.query({
        query: eventsByBand,
        variables: params,
  ...
  computed: {
    band () {
      return get(this.eventsByBand, 'band', {})
    },

    events () {
      return get(this.eventsByBand, 'events', {})
    },
    ...
    title () {
      ...
      let value = `Atlanta Events - ${this.band.name} | FunInATL`
    ...
    description () {
      ...
      let value = `Atlanta events for music band ${this.band.name}.`
  ...
  apollo: {
    eventsByBand: {
      ...
      skip () {
        return (this.hasEvents && this.hasBand && !this.isCustomDatesChoosen)
      }
    }
  },
