...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "py-3 px-4 bg-green-500 border-b border-gray-200 flex items-center justify-center gap-4 flex-wrap" }, _attrs))}>`);
      _push(ssrRenderComponent(_component_ClientOnly, null, {
        fallback: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<span class="text-sm text-white"${_scopeId}>Loading…</span>`);
          } else {
            return [
              createVNode("span", { class: "text-sm text-white" }, "Loading…")
            ];
          }
        })
      }, _parent));
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</p></div>`);
      if (__props.event.price) {
        _push(`<div class="flex justify-center mb-2"><span class="inline-block bg-gray-100 text-gray-800 text-sm font-medium px-4 py-1.5 rounded">${__props.event.price ?? ""}</span></div>`);
      } else {
        _push(`<!---->`);
      }
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.groups, (group) => {
        _push(`<!--[--><h3 class="text-3xl font-semibold text-center text-gray-800 mt-4 mb-4">${ssrInterpolate(group.label)}</h3>`);
...
//# sourceMappingURL=EventList-obfkfhQh.mjs.map
